...
- **Logo/favicon/OG-image asset set is still the old green identity:** found during the final whole-branch review — `favicon-16.png`, `favicon-32.png`, `logo-192.png`, `logo-512.png`, `logo-instagram-dp.png`, and `logo.svg` all still carry the discontinued `#8DC63F` green lightning-bolt mark. No task in this plan touched image assets, so the palette migration swept every CSS/HTML color reference but left every tab icon and every shared-link social-preview card showing the old brand. Needs the raster set regenerated in the terracotta palette (and `logo.svg`, which is otherwise orphaned/unreferenced, updated or removed) — this is new design/asset work, not a text edit, so it wasn't done as part of this plan.
